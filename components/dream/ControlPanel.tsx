    ...
    return (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-full max-w-5xl p-4 glass-panel rounded-lg flex gap-8 text-white">
            ...
            <div className="flex-1 space-y-3">
                <h3 className="font-bold mb-2">Настройки Кисти</h3>
                <div>
                     <label className="text-xs text-slate-400">Тип Кисти</label>
                     <select 
                        value={brush.type} 
                        onChange={e => setBrush(p => ({ ...p, type: e.target.value as BrushParams['type'] }))}
                        className="w-full bg-slate-800 border border-slate-700 rounded-md p-1 text-xs mt-1 outline-none focus:border-cyan-500"
                    >
                ...
                <Slider label="Интенсивность" value={brush.flow} min={0} max={1} step={0.01} onChange={v => setBrush(p => ({ ...p, flow: v }))} />
            </div>
            ...
            <div className="flex-1 space-y-4">
                <h3 className="font-bold mb-2">Физика Холста</h3>
                <Slider label="Вязкость чернил" value={simulation.viscosity} min={0} max={1} step={0.01} onChange={v => setSimulation(p => ({ ...p, viscosity: v }))} />
