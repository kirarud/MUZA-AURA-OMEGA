...
import { Mic, Power, Plus, Cpu, Sparkles, Brain } from 'lucide-react';
...
const Chronicles: React.FC<ChroniclesProps> = (props) => {
    ...
    useEffect(() => {
        const checkTokens = setInterval(() => {
            const tokens = aiService.getRecentTokens();
            if (tokens.length > 0) {
                setVisibleTokens(prev => [...prev, ...tokens].slice(-10));
                aiService.clearRecentTokens();
                setTimeout(() => setVisibleTokens([]), 4000);
            }
        }, 300);
        return () => clearInterval(checkTokens);
    }, [aiService]);
    ...
    return (
        <div className="w-full h-full flex flex-col relative overflow-hidden bg-black/50">
            {/* Neural Feed Overlay */}
            <div className="absolute top-24 left-1/2 -translate-x-1/2 flex gap-4 pointer-events-none z-40 overflow-hidden w-full justify-center">
                {visibleTokens.map((t, i) => (
                    <div key={i} className="animate-[float-up_2s_ease-out_forwards] bg-cyan-500/10 border border-cyan-400/30 px-3 py-1 rounded-full text-[11px] font-mono text-cyan-300 backdrop-blur-md shadow-[0_0_20px_rgba(34,211,238,0.2)]">
                        {t.toUpperCase()}
                    </div>
                ))}
            </div>
            ...
            <div className="flex-1 w-full max-w-4xl mx-auto pt-24 pb-2 overflow-hidden relative">
            ...
            <div className="w-full px-4 pb-8 z-10 max-w-2xl mx-auto">
                {aiService.getThinkingEnergy() > 0.1 && (
                    <div className="flex items-center gap-3 mb-4 text-[11px] font-mono text-cyan-400/60 animate-pulse px-4">
                        <Brain size={14} /> СИНТЕЗ КВАНТОВОГО ОТВЕТА...
                    </div>
                )}
            ...
            <style>{`
                @keyframes float-up {
                    ...
                    100% { transform: translateY(-100px); opacity: 0; filter: blur(10px); }
                }
            `}</style>
        </div>
    );
