...
export class OpticsEngine {
    ...
    public render(nodes: MuzaAINode[]) {
        ...
        nodes.forEach(node => {
            ...
            const energyFactor = Math.max(0, Math.min(node.hyperbits / 5, 1)); // Normalize energy effect, ensure non-negative

            // CRITICAL FIX: Ensure radius is always positive
            const radius = Math.max(0.1, node.mass * 10 * (1 + energyFactor));
            ...
            try {
                ...
                // Skip nodes that cause gradient errors
            }
        });
    }
}
